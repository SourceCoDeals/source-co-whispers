...
export default function NewTracker() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.from("industry_trackers").insert({ industry_name: name.trim(), user_id: user.id }).select().single();
      ...
      <div className="max-w-xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input id="name" placeholder="e.g., Residential Roofing, Collision Repair" value={name} onChange={(e) => setName(e.target.value)} className="mt-1" />
          </div>
          <Button type="submit" disabled={isLoading || !name.trim()}>
            {isLoading && <Loader2 className="w-4 h-4 mr-2 animate-spin" />}Create Universe
          </Button>
        </form>
      </div>
    </AppLayout>
